...
public class PlayerHitHandler : MonoBehaviour
{
    ...
    public bool isKnockBack; //특수 공격에 피격당한 상태인지
    ...
    /// 플레이어의 피격(Hit) 액션
    /// </summary>
    public void Player_ApplyDamage(int damage, bool criticalHit, bool facingRight)
    {
        //살아있는 상태에서만 피격가능
        if (!isDead && !isInvincible)
        {
            float critical_knockbackForce = facingRight ? Critcal_knockbackForce : -Critcal_knockbackForce;
            float basic_knockbackForce = facingRight ? Basic_knockbackForce : -Basic_knockbackForce;
    ...
    IEnumerator HitTime(bool _criticalHit)
    {
        if (_criticalHit)
        {
            ...
            isInvincible = true;
            yield return new WaitUntil(() => isKnockBack == false); //넉백 끝날때동안 무적상태 + 움직일수없음 + 공격불가
